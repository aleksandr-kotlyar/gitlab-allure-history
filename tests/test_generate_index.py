...
def test_generator_footer_renders_without_version(monkeypatch):
    ...
    assert f'Generated by <a href="{GENERATOR_URL}">gitlab-allure-history</a>' in html
    assert COMPONENT_URL not in html
    assert "v" not in html


def test_generator_footer_renders_with_version(monkeypatch):
    monkeypatch.setenv(VERSION_ENV, "2026.2.9")
    monkeypatch.setenv(SHOW_FOOTER_ENV, "true")
    ...
    assert (
        f'<a class="generator-version" href="{COMPONENT_URL}">2026.2.9</a>'
        in html
    )
...
def test_generator_footer_escapes_version(monkeypatch):
    ...
    assert (
        f'<a class="generator-version" href="{COMPONENT_URL}">'
